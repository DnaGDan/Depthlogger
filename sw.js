...
const ASSETS=['./','./index_pwa_v2-1.html','./manifest.json','./icon-192.png','./icon-512.png','./brownfield_logo.png'];
...
self.addEventListener('fetch',e=>{
  ...
  e.respondWith((async()=>{
    ...
    catch(err){ if(e.request.headers.get('accept')?.includes('text/html')) return caches.match('./index_pwa_v2-1.html'); throw err; }
  })());
});
});
