...
self.addEventListener('fetch',e=>{
  ...
  e.respondWith((async()=>{
    ...
    if(cached){
      // refresh cache in background but only store successful responses
      e.waitUntil(
        fetch(e.request).then(r=>{
          if (r && r.ok) return caches.open(CACHE).then(c=>c.put(e.request,r.clone()));
        }).catch(()=>{})
      );
      return cached;
    }
    try{
      ...
      if(e.request.headers.get('accept')?.includes('text/html')) return caches.match('./index_pwa_v2.html');
      throw err;
    }
  })());
});
